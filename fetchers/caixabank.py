...
BASE_URL = "https://caixabankcareers.com"
JOBS_URL = "https://caixabankcareers.com/search/?createNewAlert=false&q=&locationsearch="


def fetch(limit: int, source_name: str, **kwargs) -> list[JobPosting]:
    """
    Récupère les offres d'emploi pour CaixaBank (plateforme SuccessFactors).
    """
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # 1. Gérer la bannière de cookies en espagnol
            try:
                cookie_button = page.get_by_role('button', name='Continuar')
            ...
            while len(job_postings) < limit:
                ...
                for row in job_rows:
                    ...
                    job = JobPosting(
                        ...
                        title=title,
                        link=absolute_link,
                        posted=datetime.now(timezone.utc), # Pas de date disponible
                        source=source_name,
                        company=source_name,
                        location=location,
                    )
                ...
                try:
                    ...
                    # On cherche un lien <a> dont le texte est exactement le numéro de la page suivante
