...
/** DB minimaliste: une seule table user_data avec 2 colonnes JSON */
function dbFile() {
...
export async function GET(req: NextRequest) {
  ...
  try {
    const row = db
      .prepare("SELECT alerts_json FROM user_data WHERE user = ?")
      .get(user) as { alerts_json: string } | undefined;
    const alerts = row ? JSON.parse(row.alerts_json || "[]") : [];
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    if (exists) {
      db.prepare("UPDATE user_data SET alerts_json = ?, updated_at = ? WHERE user = ?")
        .run(json, now, user);
    } else {
