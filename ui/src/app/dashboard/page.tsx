...
type View = "favs" | "applied" | "calendar";

type Planner = { open: boolean; when: string; note: string; location: string; url: string };
...
export default function DashboardPage() {
  ...
  const [planner, setPlanner] = useState<Record<string, Planner>>({});
  ...
  const kpis = useMemo(() => {
    const rows = view === "favs" ? favs : view === "applied" ? applied : applied;
  ...
  const openPlanner = (id: string) =>
    setPlanner((m)=> ({ ...m, [id]: m[id] ?? { open:true, when: defaultWhen(), note:"", location:"", url:"" } , [id]: { ...(m[id] ?? {}), open:true } as Planner }));
  const closePlanner = (id: string) =>
    setPlanner((m)=> ({ ...m, [id]: { ...(m[id] ?? { when: defaultWhen(), note:"", location:"", url:"" }), open:false } }));

  const confirmPlanner = (j: SavedJob) => {
    const p = planner[j.id]; if (!p) return;
    const ts = new Date(p.when).getTime();
    incInterviews(j.id, +1, { ts, note: p.note, location: p.location, url: p.url });
    setItems(getAll());
    closePlanner(j.id);
  };
  const minusOne = (id: string) => { incInterviews(id, -1); setItems(getAll()); };
    ...
    <main className="container mx-auto px-4 py-10 sm:px-6 lg:px-8 space-y-8">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <motion.h1 className="text-3xl sm:text-4xl font-semibold tracking-tight neon-title" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          Dashboard{" "}
          {view === "favs" ? (<><span className="text-primary">Favoris</span> ⭐</>)
           : view === "applied" ? (<><span className="text-primary">Candidatures</span> 📄</>)
           : (<><span className="text-primary">Calendrier</span> 📅</>)}
        </motion.h1>
        <div className="flex items-center gap-2">
          <SegmentedControl
            options={[{ key: "favs", label: "Favoris ⭐" }, { key: "applied", label: "Candidatures 📄" }, { key: "calendar", label: "Calendrier 📅" }]}
            value={view}
      ...
      {view === "calendar" && (
        <CalendarMonth allJobs={applied} onChanged={() => setItems(getAll())} />
      )}

      {view !== "calendar" && prefs.showKPIs && (
                    ...
                    {reminders.map((r) => (
                      <li key={r.id} className="flex items-center justify-between gap-3 rounded border border-border px-3 py-2 hover:border-primary transition">
                        <div className="flex items-center gap-2 min-w-0">
                          <BankAvatar name={r.company ?? r.source} size={22} />
                  ...
                  (view === "favs" ? favs : applied).map((j, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-3">
                          <div className="flex items-center gap-2">
                            <BankAvatar name={j.company ?? j.source} size={26} />
                        ...
                        <td className="p-3">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              className="px-2 py-1 text-xs rounded border border-border hover:border-primary inline-flex items-center gap-1"
                              onClick={() => openPlanner(j.id)}
                              title="Planifier un entretien (+1)"
                            >
                              <Plus className="w-3.5 h-3.5" /> {j.interviews ?? 0}
                            </button>
                            <button
                              className="px-2 py-1 text-xs rounded border border-border hover:border-danger inline-flex items-center gap-1"
                              onClick={() => minusOne(j.id)}
                              title="Retirer le dernier entretien (–1)"
                            >
                          ...
                          {p?.open && (
                            <div className="mt-3 rounded-lg border border-border bg-card p-3 flex flex-col lg:flex-row gap-2">
                              <input
                                ...
                                value={p.when}
                                onChange={(e)=>setPlanner((m)=>({ ...m, [j.id]: { ...(m[j.id] as Planner), when: e.target.value } }))}
                              />
                              <input
                                type="text"
                                placeholder="Note"
                                ...
                                value={p.note}
                                onChange={(e)=>setPlanner((m)=>({ ...m, [j.id]: { ...(m[j.id] as Planner), note: e.target.value } }))}
                              />
                              <input
                                type="text"
                                ...
                                className="bg-surface border border-border rounded px-2 h-10"
                                value={p.location}
                                onChange={(e)=>setPlanner((m)=>({ ...m, [j.id]: { ...(m[j.id] as Planner), location: e.target.value } }))}
                              />
                              <input
                                type="url"
                                ...
                                className="bg-surface border border-border rounded px-2 h-10"
                                value={p.url}
                                onChange={(e)=>setPlanner((m)=>({ ...m, [j.id]: { ...(m[j.id] as Planner), url: e.target.value } }))}
                              />
                              <div className="flex items-center gap-2">
                                <button className="h-10 px-3 rounded-lg border border-border hover:border-primary" onClick={()=>confirmPlanner(j)}>
                                  Confirmer
                                </button>
                                <button className="h-10 px-3 rounded-lg border border-border hover:border-danger" onClick={()=>closePlanner(j.id)}>
                                  Annuler
                                </button>
                              </div>
                            </div>
    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-64 rounded-xl border border-border bg-card p-3 z-10 shadow-[var(--glow-weak)]">
          {[["showKPIs","KPIs"],["showTopByBank","Top banques"],["showTimeSeries","Time series"],["showReminders","Rappels (7j)"]].map(([k,label]) => (
