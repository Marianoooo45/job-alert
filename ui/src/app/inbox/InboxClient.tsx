...
export default function InboxClient() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const res = await fetch(
        `/api/jobs?${qsFromAlert(selected).toString()}`,
        { cache: "no-store" }
      );
  ...
  return (
    <main className="container mx-auto px-4 py-8 sm:px-6 lg:px-8">
      <h1 className="text-3xl font-semibold tracking-tight neon-title mb-6">
        Inbox
      </h1>
                    ...
                    <div className="truncate text-xs text-muted-foreground">
                      {[
                        Alerts.normalizeKeywords(a.query.keywords)?.length
                          ? `#${Alerts.normalizeKeywords(a.query.keywords)!.join(
                              " #"
                            )}`
                          : null,
                        a.query.banks?.length
                          ? `${a.query.banks.length} banque(s)`
                          : null,
                        a.query.categories?.length
                          ? `${a.query.categories.length} métier(s)`
                          : null,
                        a.query.contractTypes?.length
                          ? `${a.query.contractTypes.length} contrat(s)`
                          : null,
                      ]
                        .filter(Boolean)
                        .join(" • ")}
                        ...
                        <TableCell className="align-top">
                          <div className="flex items-center gap-2">
                            <Link
                              ...
                              onClick={() => {
                                if (selected)
                                  Alerts.markJobSeen(selected.id, job.id);
                              }}
                            >
                              {job.title}
                            </Link>
