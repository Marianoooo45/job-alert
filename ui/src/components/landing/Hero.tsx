    ...
    >
      ...
      <motion.div style={{ y: yMedia }} className="absolute inset-0">
        <video
          ...
          style={{ filter: "var(--hero-media-filter)" }}
          autoPlay
          loop
          muted
          playsInline
          ...
          onError={(e) => {
            ...
            img.style.cssText = "width:100%;height:100%;object-fit:cover;filter:var(--hero-media-filter)";
      ...
      {/* Scrim unique piloté par les variables de thème */}
      <motion.div
        style={{ y: yOverlay }}
        className="absolute inset-0 [background:var(--hero-scrim)]"
      />
      ...
      <div className="relative z-10 p-6 sm:p-10 flex flex-col gap-4 sm:gap-6 max-w-3xl">
        ...
        <motion.p
          className="hero-sub text-lg sm:text-xl text-muted-foreground/90"
          initial={{ opacity: 0, y: 8 }}
          animate={{ opacity: 1, y: 0 }}
