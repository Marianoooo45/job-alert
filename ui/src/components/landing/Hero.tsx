...
export default function Hero() {
  ...
  return (
    <section ref={ref} className="hero-section relative rounded-3xl overflow-hidden border border-border panel-xl min-h-[56vh] sm:min-h-[62vh]">
      {/* MEDIA */}
      <motion.div style={{ y: yMedia }} className="hero-media absolute inset-0 z-0">
        ...
        <div className="media-dark absolute inset-0">
          ...
          <video className="absolute inset-0 w-full h-full object-cover" autoPlay loop muted playsInline preload="metadata" poster={HERO_POSTER}
        ...
        {/* Light = image jour */}
        <img src={HERO_DAY} alt="City skyline (day)" className="media-light absolute inset-0 w-full h-full object-cover" />
      </motion.div>

      {/* SCRIMS */}
      <motion.div style={{ y: yOverlay }} className="hero-scrim absolute inset-0 z-[1]" />
      <div className="absolute inset-0 z-[1] bg-[linear-gradient(180deg,rgba(11,14,20,.30),transparent_40%,rgba(11,14,20,.55)_100%)]" />

      {/* CONTENU */}
      <div className="relative z-[2] p-6 sm:p-10 max-w-3xl flex flex-col gap-4 sm:gap-6">
        <motion.h1 className="hero-title text-4xl sm:text-6xl font-semibold tracking-tight"
        ...
        <motion.p className="hero-sub text-lg sm:text-xl"
          initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .45, delay: .08 }}>
          Offres centralisées, alertes intelligentes, suivi de candidatures. Conçu pour les étudiants et jeunes diplômés.
        </motion.p>
        <motion.div className="flex items-center gap-3"
          initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .45, delay: .16 }}>
